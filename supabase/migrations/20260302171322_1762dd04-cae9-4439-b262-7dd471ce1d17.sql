...
BEGIN
    ...
    Ventas AS (
        SELECT p.title AS p, SUM(oi.quantity::BIGINT) as uv,
               SUM(CASE
                 WHEN COALESCE(oi.manual_discount_amount::NUMERIC, 0) = 0
                 ...
                 THEN oi.quantity ELSE 0 END) as uf,
               SUM(CASE
                 WHEN COALESCE(oi.manual_discount_amount::NUMERIC, 0) = 0
                  AND (
                 ...
                 THEN oi.quantity ELSE 0 END) as ur,
    ...
    SELECT B.f, 'Varias Tallas'::TEXT, B.p, B.c, B.u_vendidas::BIGINT, B.s_tiendas::BIGINT, B.s_digital::BIGINT,
           CASE WHEN B.u_full_price >= (B.u_rebajas + B.u_promo) THEN '🟢 Venta Full'
                ELSE '🔴 Venta con Impulso' END::TEXT,
