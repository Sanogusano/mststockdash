...
AS $$
  WITH bounds AS (
    SELECT
      date_trunc('month', p_mes)::timestamp with time zone AS desde,
      (date_trunc('month', p_mes) + interval '1 month')::timestamp with time zone AS hasta
  ), base AS (
    SELECT
      at2.canal,
      at2.tipo_de_venta,
    ...
    WHERE at2.fecha_creacion >= b.desde
      AND at2.fecha_creacion < b.hasta
