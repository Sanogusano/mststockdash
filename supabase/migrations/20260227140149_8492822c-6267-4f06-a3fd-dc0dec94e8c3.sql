...
RETURNS TABLE(
  categoria text,
  stock_tiendas bigint,
  stock_digital bigint,
  und_tiendas bigint,
  und_outlets bigint,
  und_digital bigint,
  und_total bigint,
...
BEGIN
  -- Calculate grand total units for participation %
  ...
  SELECT
    ...
    COALESCE(st.st, 0)::BIGINT,
    COALESCE(sd.sd, 0)::BIGINT,
    COALESCE(v.uds_tiendas, 0)::BIGINT,
    COALESCE(v.uds_outlets, 0)::BIGINT,
    COALESCE(v.uds_digital, 0)::BIGINT,
    COALESCE(v.uds_total, 0)::BIGINT,
    ROUND(COALESCE(v.uds_total, 0)::NUMERIC / NULLIF(gran_total_uds, 0) * 100, 1)::NUMERIC,
    -- Sell-through: vendidas / (vendidas + stock_total)
