...
-- 1) Add optional payment parameters (for in-kind bonuses)
ALTER TABLE public.incentivo_recompensas
  ADD COLUMN IF NOT EXISTS parametros_pago jsonb NOT NULL DEFAULT '{}'::jsonb;

-- 2) Replace RPC actualizar_progreso_incentivo adding branch for tienda_cumplimiento
...
BEGIN
    ...
    -------------------------------------------------------------------
    -- NUEVO: TIENDA CUMPLIMIENTO (UPT / % Full Price / Ticket Promedio)
    -------------------------------------------------------------------
    IF v_regla.tipo_regla = 'tienda_cumplimiento' THEN
        ...
        SELECT
            ...
            -- cumple según operador
            CASE
                WHEN NOT (v_upt_activa OR v_fp_activa OR v_tk_activa) THEN false
                WHEN v_operador = 'OR' THEN
                    (v_upt_activa AND e.upt >= v_upt_min)
                    OR (v_fp_activa AND e.full_price_pct >= v_fp_min)
                    OR (v_tk_activa AND e.ticket_promedio >= v_tk_min)
                ELSE
                    (NOT v_upt_activa OR e.upt >= v_upt_min)
                    AND (NOT v_fp_activa OR e.full_price_pct >= v_fp_min)
                    AND (NOT v_tk_activa OR e.ticket_promedio >= v_tk_min)
            END AS cumple,
            0::numeric AS placeholder_monto,
            now()
        FROM eval e;

        -- Set monto_ganado post-insert (bono en especie => 0, monto/bono_monto/monto_fijo => valor)
    ...
    -------------------------------------------------------------------
    -- (Sin cambios) presupuesto_semanal_dual
    -------------------------------------------------------------------
    ...
    IF v_regla.tipo_regla IN ('ticket_minimo', 'upt_minimo', 'numero_pedidos') THEN
        ...
        SELECT v_inc.id, user_id, location_id,
            ...
            now()
        FROM califican GROUP BY user_id, location_id;
        RETURN;
    END IF;
END;
$function$;
