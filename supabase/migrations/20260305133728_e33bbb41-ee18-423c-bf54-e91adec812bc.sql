...
BEGIN
    ...
    -- For INSUMOS: use the most recent snapshot that has insumo data
    SELECT MAX(inv.snapshot_date) INTO v_max_date_insumos
    FROM inventory_snapshot inv
    JOIN product_catalog pc ON pc.variant_id = inv.variant_id
    WHERE UPPER(pc.category) IN ('BOLSA', 'INSUMOS');
    ...
    StockPorTienda AS (
        ...
        -- INSUMOS stock from most recent snapshot (may differ from prendas date)
        SELECT cm.tipo_inv,
               inv.location_id AS loc_id,
               SUM(inv.available)::BIGINT AS stock_total
        FROM inventory_snapshot inv
