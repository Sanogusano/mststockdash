-- Performance index for current inventory join key
CREATE INDEX IF NOT EXISTS idx_inv_snapshot_date_variant ON inventory_snapshot (snapshot_date, variant_id);

-- Fix inventory health RPC: inventory_snapshot now matches by variant_id (sku is null in latest snapshots)
