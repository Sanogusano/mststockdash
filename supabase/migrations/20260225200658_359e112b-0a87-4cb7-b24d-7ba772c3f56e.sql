...
BEGIN
    ...
    WosPorTienda AS (
        SELECT
            ...
            -- Consumo diario del origen (para calcular retención de 60 días)
    ...
    -- Destinos: tiendas con RIESGO (WOS < 4 y venden)
    Destinos AS (
    ...
    -- Origenes: tiendas con SOBRESTOCK (WOS > 12) y stock > 3
    -- Además el origen debe poder quedarse con al menos 60 días de autonomía después de ceder stock
    -- stock_cedible = stock - (consumo_diario * 60)  → lo que puede dar sin bajar de 60 días
    Origenes AS (
        SELECT w.producto, w.location_id, w.stock, w.wos, w.consumo_diario,
               GREATEST(w.stock - CEIL(w.consumo_diario * 60), 0) AS stock_cedible
        FROM WosPorTienda w
        WHERE w.wos > 12
          AND w.stock > 3
          AND (w.stock - CEIL(w.consumo_diario * 60)) > 0  -- sólo si realmente puede ceder
    ),
    -- Cruzar destinos con orígenes, rankear por mayor stock cedible y elegir SOLO el mejor origen por producto+destino
    Candidatos AS (
        SELECT 
            d.foto,
            d.producto,
            ...
            ROW_NUMBER() OVER (PARTITION BY d.producto, d.location_id ORDER BY ori.stock_cedible DESC) AS rn
        FROM Destinos d
        JOIN Origenes ori ON d.producto = ori.producto AND d.location_id != ori.location_id
    ),
    -- Sólo 1 origen por producto+destino (el que más stock puede ceder)
    Unicos AS (
