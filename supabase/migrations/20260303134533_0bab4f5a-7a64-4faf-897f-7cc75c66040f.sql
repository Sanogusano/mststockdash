...
CREATE OR REPLACE FUNCTION public.reporte_metricas_zona(
  dias_atras integer,
...
RETURNS TABLE(
  ...
  venta_promedio_semana numeric,
  venta_promedio_finde numeric
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  periodo_actual AS (
    ...
    WHERE o.created_at >= (NOW() - (GREATEST(COALESCE(dias_atras, 1), 1) || ' days')::INTERVAL)
      AND o.location_id IN (SELECT location_id FROM locs)
