...
BEGIN
    ...
    RETURN QUERY
    WITH
    CategoryMapping AS MATERIALIZED (
    ...
    -- Single scan of inventory snapshot: filter by dates FIRST, then join catalog
    StockPorTienda AS (
        SELECT cm.tipo_inv, inv.location_id AS loc_id, SUM(inv.available)::BIGINT AS stock_total
        FROM (
            SELECT variant_id, location_id, available, snapshot_date
            FROM inventory_snapshot
            WHERE (v_max_date_prendas = v_max_date_insumos AND snapshot_date = v_max_date_prendas)
               OR (v_max_date_prendas <> v_max_date_insumos AND snapshot_date IN (v_max_date_prendas, v_max_date_insumos))
        ) inv
        JOIN CategoryMapping cm ON cm.variant_id = inv.variant_id
        WHERE (cm.tipo_inv = 'PRENDAS' AND inv.snapshot_date = v_max_date_prendas)
           OR (cm.tipo_inv = 'BOLSAS Y EMPAQUES' AND inv.snapshot_date = v_max_date_insumos)
