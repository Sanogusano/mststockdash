...
BEGIN
  ...
  VentasPorTienda AS (
    ...
    WHERE o.created_at >= (NOW() - (GREATEST(COALESCE(dias_atras, 1), 1) || ' days')::INTERVAL)
      AND UPPER(COALESCE(p.category,'')) NOT IN ('BOLSA', 'INSUMOS')
    GROUP BY p.product_id, p.title, p.color, oi.sku, REGEXP_REPLACE(COALESCE(o.location_id, ''), '^gid://shopify/Location/', '')
  ),
  StockPorTienda AS (
    SELECT
      p.product_id AS pid,
    ...
    WHERE inv.snapshot_date = v_max_date
      AND UPPER(COALESCE(p.category,'')) NOT IN ('BOLSA', 'INSUMOS')
    GROUP BY p.product_id, p.title, p.color, inv.sku, REGEXP_REPLACE(COALESCE(inv.location_id, ''), '^gid://shopify/Location/', '')
  ),
  Combined AS (
    SELECT
