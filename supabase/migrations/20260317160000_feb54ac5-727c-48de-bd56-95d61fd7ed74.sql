...
BEGIN
  ...
  ventas_clasificadas AS (
    SELECT
      CASE
        WHEN vn.source_name = 'pos' AND l.location_id IS NOT NULL THEN l.name
        WHEN vn.source_name = 'shopify_draft_order' THEN 'Personal Shopper'
        ELSE 'Tienda Online'
      END AS nombre,
      CASE
        WHEN vn.source_name = 'pos' AND l.location_id IS NOT NULL THEN 'tienda'
        ELSE 'canal'
      END AS tipo,
      l.zona,
