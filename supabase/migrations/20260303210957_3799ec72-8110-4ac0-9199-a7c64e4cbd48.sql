...
RETURNS TABLE(
  product_id text, producto text, color text, foto text, talla text,
  sku text, tienda_destino text, stock_destino bigint, ritmo_venta numeric,
  uds_sugeridas bigint, tienda_origen text, stock_origen bigint, prioridad numeric
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  inv AS (
    SELECT
      i.sku,
      REGEXP_REPLACE(i.location_id, '^gid://shopify/Location/', '') AS loc_id,
      SUM(i.available)::BIGINT AS stock
    FROM inventory_snapshot i
    WHERE i.snapshot_date = v_max_date
      AND i.available > 0
    GROUP BY i.sku, REGEXP_REPLACE(i.location_id, '^gid://shopify/Location/', '')
  ),
  ventas AS (
    SELECT
      oi.sku,
