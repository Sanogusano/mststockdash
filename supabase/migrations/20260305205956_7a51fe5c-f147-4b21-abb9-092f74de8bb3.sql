...
BEGIN
  ...
  FROM (
    SELECT s.snapshot_date, COUNT(DISTINCT s.variant_id) AS cnt
  ...
  -- Sales grouped by variant_id + location
  ventas AS (
    SELECT
  ...
  -- Variant → product_id mapping
  var_product AS (
    SELECT DISTINCT pc2.variant_id AS vid, pc2.product_id AS pid
    FROM product_catalog pc2
    WHERE pc2.variant_id IS NOT NULL AND pc2.product_id IS NOT NULL
  ),
  -- Find trigger variants: WOS < 4 with sales at a destination
  trigger_variants AS (
    SELECT c.vid, c.loc_id, vp.pid
    FROM combined c
    JOIN var_product vp ON c.vid = vp.vid
    WHERE c.wos > 0 AND c.wos < 4 AND c.vps > 0
  ),
  -- Expand to ALL sibling variants of triggered products at same destination
  product_dest_triggers AS (
    SELECT DISTINCT tv.pid, tv.loc_id AS dest_loc
    FROM trigger_variants tv
  ),
  -- All variants for triggered products
  sibling_variants AS (
    SELECT DISTINCT vp.vid, vp.pid
    FROM var_product vp
    WHERE vp.pid IN (SELECT pdt.pid FROM product_dest_triggers pdt)
  ),
  -- For each sibling variant at each triggered destination, compute need
  destinos_expanded AS (
    SELECT
      sv.vid,
      sv.pid,
      pdt.dest_loc AS loc_id,
      COALESCE(c.stock, 0)::BIGINT AS stock,
      COALESCE(c.vps, 0) AS vps,
      -- Need: at least 2 units per size if stock < 2
      GREATEST(2 - COALESCE(c.stock, 0)::BIGINT, 0)::BIGINT AS need
    FROM sibling_variants sv
    CROSS JOIN product_dest_triggers pdt
    JOIN var_product vp2 ON sv.vid = vp2.vid AND sv.pid = pdt.pid
    LEFT JOIN combined c ON sv.vid = c.vid AND pdt.dest_loc = c.loc_id
    WHERE COALESCE(c.stock, 0) < 2
  ),
  -- Origins: locations with surplus for each variant
  origenes AS (
    SELECT
  ...
  -- Match destinations with origins
  candidatos AS (
    SELECT
      d.vid, d.pid, d.loc_id AS dest_loc, d.stock AS stock_dest, d.vps,
      d.need,
      o.loc_id AS orig_loc, o.stock AS stock_orig, o.stock_cedible,
      LEAST(o.stock_cedible, d.need)::BIGINT AS uds,
      ROW_NUMBER() OVER (PARTITION BY d.vid, d.loc_id ORDER BY o.stock_cedible DESC) AS rn
    FROM destinos_expanded d
    JOIN origenes o ON d.vid = o.vid AND d.loc_id <> o.loc_id
    WHERE o.stock_cedible > 0 AND d.need > 0
  ),
  -- Category participation for priority
  cat_part AS (
    SELECT
  ...
  final AS (
    SELECT
      ...
      ROUND(COALESCE(cpct.pct, 1) * (1.0 / NULLIF(
        CASE WHEN ca.vps = 0 THEN 999 ELSE ROUND(ca.stock_dest::NUMERIC / NULLIF(ca.vps / 7.0, 0), 1) END
      , 0)), 4) AS prioridad
    FROM candidatos ca
  ...
  FROM final f
  ORDER BY f.product_id, f.tienda_destino, f.prioridad DESC
  LIMIT 1000;
END;
$$;
