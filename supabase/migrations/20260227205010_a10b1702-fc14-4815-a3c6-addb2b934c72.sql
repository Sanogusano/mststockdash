...
DROP FUNCTION IF EXISTS public.reporte_salud_inventario(integer);
...
BEGIN
    ...
    StockPorTienda AS (
        SELECT cm.tipo_inv, inv.location_id, SUM(inv.available) as stock_total
    ...
    LEFT JOIN StockPorTienda s ON l.location_id = s.location_id AND t.tipo_inv = s.tipo_inv
