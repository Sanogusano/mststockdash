...
DECLARE
  v_max_date date;
BEGIN
    -- Use the most recent snapshot with full coverage (>=5000 distinct variants)
    SELECT snapshot_date INTO v_max_date
    FROM (
    ...
    -- Fallback to absolute max if no full snapshot found
