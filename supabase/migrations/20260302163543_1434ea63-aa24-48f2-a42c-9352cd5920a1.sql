  ...
  WITH items_clasificados AS (
    SELECT
      ...
      CASE
        ...
        -- Prioridad 2a: Rebajas (compare_at_price > price, fallback a product_catalog)
        WHEN COALESCE(NULLIF(oi.compare_at_price::numeric, 0), NULLIF(p.compare_at_price::numeric, 0), 0) > oi.price::numeric THEN 'REBAJAS'
        -- Prioridad 2b: Rebajas ocultas (item vendido por debajo del precio de catálogo sin compare_at_price)
