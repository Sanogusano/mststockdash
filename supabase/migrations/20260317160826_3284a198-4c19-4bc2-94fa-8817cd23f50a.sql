...
BEGIN
  ...
  ventas_clasificadas AS (
    SELECT
      CASE
        -- Tiendas físicas: solo POS y NO el CEDI
        WHEN vn.source_name = 'pos' AND vn.location_id <> '71474315479' AND l.location_id IS NOT NULL
          THEN l.name
        -- Personal Shopper: draft orders
        WHEN vn.source_name = 'shopify_draft_order'
          THEN 'Personal Shopper'
        -- Tienda Online: web, 580111, o POS del CEDI, o cualquier otro source
        ELSE 'Tienda Online'
      END AS nombre,
      CASE
        WHEN vn.source_name = 'pos' AND vn.location_id <> '71474315479' AND l.location_id IS NOT NULL
