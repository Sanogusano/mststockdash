...
BEGIN
  ...
  RETURN QUERY
  WITH
  -- Classify current month sales by channel (same logic as calcular_proyecciones)
  ventas_items AS (
    SELECT
      ...
      oi.quantity,
      oi.price,
      COALESCE(oi.manual_discount_amount, 0) AS disc,
      COALESCE(oi.category, '') AS cat
    FROM orders o
  ...
  ventas_clasificadas AS (
    SELECT
      CASE
        WHEN vi.source_name = 'pos' AND vi.loc_id <> '71474315479' THEN
          (SELECT l.name FROM locations l WHERE l.location_id = vi.loc_id AND l.is_active = true LIMIT 1)
        WHEN vi.source_name = 'shopify_draft_order' THEN 'Personal Shopper'
        ELSE 'Tienda Online'
      END AS canal_nombre,
      vi.shopify_order_id,
      vi.quantity,
  ...
  -- Aggregate per channel: sales (excl BOLSA/INSUMO), pedidos (ALL), units (excl BOLSA/INSUMO)
  va AS (
    SELECT
      vc.canal_nombre,
      SUM(CASE WHEN vc.cat NOT IN ('BOLSA','INSUMO') THEN (vc.price * vc.quantity - vc.disc) / 1.19 ELSE 0 END) AS venta,
      SUM(CASE WHEN vc.cat NOT IN ('BOLSA','INSUMO') THEN vc.price * vc.quantity / 1.19 ELSE 0 END) AS bruta,
      SUM(CASE WHEN vc.cat NOT IN ('BOLSA','INSUMO') THEN vc.disc / 1.19 ELSE 0 END) AS desc_total,
      COUNT(DISTINCT vc.shopify_order_id) AS pedidos,
      SUM(CASE WHEN vc.cat NOT IN ('BOLSA','INSUMO') THEN vc.quantity ELSE 0 END) AS unidades
    FROM ventas_clasificadas vc
    WHERE vc.canal_nombre IS NOT NULL
    GROUP BY vc.canal_nombre
  ),
  -- Previous month (same elapsed days) - same channel classification
  vm_items AS (
    SELECT
      CASE
  ...
  -- YoY (same elapsed days, same month previous year)
  vy_items AS (
    SELECT
      CASE
        WHEN o.source_name = 'pos' AND o.location_id <> '71474315479' THEN
  ...
  -- Post-stamp variation: need location-level data for stamps
  stamp_pre_post AS (
    SELECT
      st.location_name,
      -- Pre-stamp daily avg
      COALESCE((
        SELECT SUM(
          CASE WHEN COALESCE(oi2.category,'') NOT IN ('BOLSA','INSUMO')
               THEN (oi2.price * oi2.quantity - COALESCE(oi2.manual_discount_amount,0)) / 1.19
               ELSE 0 END
        ) / GREATEST(1, (st.s_at::date - v_inicio)::int)
        FROM orders o2
        JOIN order_items oi2 ON oi2.shopify_order_id = o2.shopify_order_id
        WHERE (
          -- Map store name back to sales
          CASE
      ...
      -- Post-stamp daily avg
      COALESCE((
        SELECT SUM(
          CASE WHEN COALESCE(oi3.category,'') NOT IN ('BOLSA','INSUMO')
               THEN (oi3.price * oi3.quantity - COALESCE(oi3.manual_discount_amount,0)) / 1.19
               ELSE 0 END
  ...
  SELECT
    ...
    CASE WHEN p.monto > 0 AND v_dias_trans > 0 THEN
      (COALESCE(v.venta,0) + (COALESCE(v.venta,0) / v_dias_trans * v_dias_rest * 0.85)) / p.monto
    ELSE 0 END,
    -- crecimiento_mom
    ...
    ELSE 0 END,
    -- upt (units excl BOLSA/INSUMO / ALL orders)
    CASE WHEN COALESCE(v.pedidos,0) > 0 THEN
    ...
    st.s_at,
    -- stamp_variacion
    CASE WHEN sp.avg_pre > 0 THEN
      (sp.avg_post - sp.avg_pre) / sp.avg_pre
    ELSE 0 END
  FROM pres p
  LEFT JOIN va v ON v.canal_nombre = p.nombre_identificador
  LEFT JOIN vm m ON m.canal_nombre = p.nombre_identificador
  LEFT JOIN vy y ON y.canal_nombre = p.nombre_identificador
  LEFT JOIN stmp st ON st.location_name = p.nombre_identificador
  LEFT JOIN stamp_pre_post sp ON sp.location_name = p.nombre_identificador;
END;
$$;
