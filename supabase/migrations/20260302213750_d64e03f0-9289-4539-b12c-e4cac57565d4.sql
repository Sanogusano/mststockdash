...
BEGIN
  ...
  RETURN QUERY
  WITH 
  -- Latest inventory by SKU per location (fast: single date)
  latest_inv AS (
    SELECT i.sku, i.location_id, SUM(i.available)::INTEGER AS available
  ...
  -- Stock status per SKU per location (only SKUs with global sales)
  stock_status AS (
    SELECT
    ...
    LEFT JOIN sales_vel sv ON sv.sku = p.sku AND sv.location_id = li.location_id
  ),
  -- Destinations: stock < 2 per SKU
  destinos AS (
    SELECT ss.product_id, ss.location_id, ss.tienda, ss.sku, ss.talla,
  ...
  origenes AS (
    SELECT ss.product_id, ss.location_id, ss.tienda, ss.sku, ss.stock, ss.venta_sem,
  ...
  matches AS (
    SELECT
      ...
      ROW_NUMBER() OVER (
        PARTITION BY d.sku, d.location_id
