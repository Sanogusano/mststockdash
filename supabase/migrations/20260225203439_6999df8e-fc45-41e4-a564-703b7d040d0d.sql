...
BEGIN
    ...
    -- Destinos: tiendas con RIESGO (WOS < 4, con ventas activas)
    Destinos AS (
    ...
    -- Origenes: SOLO tiendas con SOBRESTOCK REAL (WOS entre 12 y 998, es decir con ventas activas)
    -- Excluye estancados (WOS=999) porque ese stock no rota
    Origenes AS (
        ...
        FROM WosPorTienda w
        WHERE w.wos > 12
          AND w.wos < 999   -- excluir estancados
          AND w.stock > 3
