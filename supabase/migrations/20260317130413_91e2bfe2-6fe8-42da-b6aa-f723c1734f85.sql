...
-- ============================================================
-- Helper: returns a timestamptz representing midnight (Colombia)
-- of "today minus N days" in America/Bogota timezone.
-- ============================================================
CREATE OR REPLACE FUNCTION public._col_date_boundary(dias integer)
RETURNS timestamptz
LANGUAGE sql STABLE
SET search_path = public
AS $$
  SELECT ((NOW() AT TIME ZONE 'America/Bogota')::date - dias)::timestamp AT TIME ZONE 'America/Bogota';
$$;

-- ============================================================
-- 1. reporte_ejecutivo_kpis
-- ============================================================
CREATE OR REPLACE FUNCTION public.reporte_ejecutivo_kpis(dias_atras integer, canal_filtro text DEFAULT NULL::text, location_filtro text DEFAULT NULL::text)
...
BEGIN
  ...
  WITH ordenes_base AS (
    ...
    JOIN product_catalog p ON oi.sku = p.sku
    WHERE o.created_at >= _col_date_boundary(GREATEST(COALESCE(dias_atras, 1), 1))
      AND UPPER(p.category) NOT IN ('BOLSA', 'INSUMOS')
...
AS $function$
DECLARE
  v_dias int := GREATEST(COALESCE(dias_atras, 1), 1);
BEGIN
  RETURN QUERY
  WITH ordenes_base AS (
    SELECT o.shopify_order_id,
    ...
    JOIN locations l ON o.location_id = l.location_id
    JOIN product_catalog p ON oi.sku = p.sku
    WHERE o.created_at >= _col_date_boundary(v_dias * 2)
      AND o.created_at < _col_date_boundary(v_dias)
      AND UPPER(p.category) NOT IN ('BOLSA', 'INSUMOS')
      AND (NULLIF(TRIM(p_location_id), '') IS NULL OR o.location_id = p_location_id)
      AND (NULLIF(TRIM(p_zona), '') IS NULL OR o.location_id IN (SELECT loc.location_id FROM locations loc WHERE loc.zona = p_zona AND loc.is_active = true))
      AND (
...
-- ============================================================
-- 4. reporte_kpis_por_rango
-- ============================================================
CREATE OR REPLACE FUNCTION public.reporte_kpis_por_rango(p_desde date, p_hasta date, p_canal text DEFAULT NULL::text, p_location_id text DEFAULT NULL::text, p_zona text DEFAULT NULL::text)
 RETURNS TABLE(total_pedidos bigint, unidades_vendidas bigint, ingresos_netos numeric, ticket_promedio numeric, upt numeric, pct_pedidos_full_price numeric, pct_pedidos_rebajas numeric, pct_pedidos_con_descuento numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  WITH ordenes_base AS (
    ...
    JOIN locations l ON o.location_id = l.location_id
    JOIN product_catalog p ON oi.sku = p.sku
    WHERE o.created_at >= p_desde::timestamp AT TIME ZONE 'America/Bogota'
      AND o.created_at < (p_hasta + interval '1 day')::timestamp AT TIME ZONE 'America/Bogota'
      AND UPPER(p.category) NOT IN ('BOLSA', 'INSUMOS')
      AND (NULLIF(TRIM(p_location_id), '') IS NULL OR o.location_id = p_location_id)
      AND (NULLIF(TRIM(p_zona), '') IS NULL OR o.location_id IN (SELECT loc.location_id FROM locations loc WHERE loc.zona = p_zona AND loc.is_active = true))
      AND (
...
-- ============================================================
-- 6. reporte_ranking_tiendas_anterior
-- ============================================================
...
-- ============================================================
-- 8. reporte_composicion_coleccion
-- ============================================================
CREATE OR REPLACE FUNCTION public.reporte_composicion_coleccion(dias_atras integer, p_canal text DEFAULT NULL::text, p_location_id text DEFAULT NULL::text, p_zona text DEFAULT NULL::text)
...
-- ============================================================
-- 9. reporte_composicion_coleccion_linea
-- ============================================================
CREATE OR REPLACE FUNCTION public.reporte_composicion_coleccion_linea(dias_atras integer, p_canal text DEFAULT NULL::text, p_location_id text DEFAULT NULL::text, p_zona text DEFAULT NULL::text)
...
DECLARE
    gran_total NUMERIC;
    v_boundary timestamptz := _col_date_boundary(GREATEST(COALESCE(dias_atras, 1), 1));
BEGIN
    ...
    WHERE o.created_at >= v_boundary AND UPPER(c.category) NOT IN ('BOLSA', 'INSUMOS')
      AND (NULLIF(TRIM(p_location_id), '') IS NULL OR o.location_id = p_location_id)
      AND (NULLIF(TRIM(p_canal), '') IS NULL OR
        (LOWER(p_canal) LIKE '%digital%' AND (o.location_id = '71474315479' OR o.source_name != 'pos')) OR
        (LOWER(p_canal) LIKE '%outlet%' AND o.source_name = 'pos' AND UPPER(COALESCE(l.tipo_tienda, '')) = 'OUTLET') OR
...
-- ============================================================
-- 11. reporte_desempeño_comercial
-- ============================================================
...
BEGIN
    ...
    FROM order_items oi JOIN orders o ON oi.shopify_order_id=o.shopify_order_id
    WHERE o.created_at >= _col_date_boundary(dias_atras) AND UPPER(oi.category) NOT IN ('BOLSA','INSUMOS');
...
-- ============================================================
-- 12. reporte_desempeño_por_canal
-- ============================================================
...
-- ============================================================
-- 13. reporte_top_productos_global
-- ============================================================
...
-- ============================================================
-- 14. reporte_ejecutivo_productos
-- ============================================================
...
DECLARE v_max_date date;
  v_boundary timestamptz := _col_date_boundary(dias_atras);
BEGIN
  SELECT sub.snapshot_date INTO v_max_date
  FROM (SELECT snapshot_date, COUNT(DISTINCT variant_id) as cnt FROM inventory_snapshot GROUP BY snapshot_date ORDER BY snapshot_date DESC) sub
  WHERE sub.cnt >= 5000 LIMIT 1;
...
-- ============================================================
-- 15. reporte_metricas_zona
-- ============================================================
CREATE OR REPLACE FUNCTION public.reporte_metricas_zona(dias_atras integer, p_canal text DEFAULT NULL::text, p_zona text DEFAULT NULL::text)
...
DECLARE
  v_boundary timestamptz := _col_date_boundary(GREATEST(COALESCE(dias_atras, 1), 1));
  v_boundary_ant timestamptz := _col_date_boundary(GREATEST(COALESCE(dias_atras, 1), 1) * 2);
BEGIN
  RETURN QUERY
  WITH locs AS (
  ...
  periodo_actual AS (
    ...
    JOIN product_catalog p ON oi.sku = p.sku
    WHERE o.created_at >= v_boundary AND o.location_id IN (SELECT location_id FROM locs)
      AND UPPER(p.category) NOT IN ('BOLSA', 'INSUMOS')
  ...
  total_actual AS (
    ...
    FROM orders o JOIN order_items oi ON oi.shopify_order_id = o.shopify_order_id
    JOIN product_catalog p ON oi.sku = p.sku
    WHERE o.created_at >= v_boundary AND o.location_id IN (SELECT location_id FROM locs) AND UPPER(p.category) NOT IN ('BOLSA', 'INSUMOS')
  ),
  total_anterior AS (
    SELECT SUM(((oi.price::NUMERIC * oi.quantity::NUMERIC) - COALESCE(oi.manual_discount_amount::NUMERIC, 0)) / 1.19) / NULLIF(dias_atras::NUMERIC, 0) AS avg_venta,
      COUNT(DISTINCT o.shopify_order_id)::NUMERIC / NULLIF(dias_atras::NUMERIC, 0) AS avg_pedidos,
...
-- ============================================================
-- 16. reporte_metricas_tienda_individual
-- ============================================================
...
BEGIN
  ...
  WITH periodo_actual AS (
    ...
    FROM orders o JOIN order_items oi ON oi.shopify_order_id = o.shopify_order_id
    JOIN product_catalog p ON oi.sku = p.sku
    WHERE o.created_at >= v_boundary AND o.location_id = p_location_id AND UPPER(p.category) NOT IN ('BOLSA', 'INSUMOS')
    GROUP BY EXTRACT(DOW FROM o.created_at AT TIME ZONE 'America/Bogota'), TRIM(TO_CHAR(o.created_at AT TIME ZONE 'America/Bogota', 'Day'))
  ),
  mejor AS (SELECT dia_nombre AS dia, venta FROM periodo_actual ORDER BY venta DESC LIMIT 1),
  peor AS (SELECT dia_nombre AS dia, venta FROM periodo_actual ORDER BY venta ASC LIMIT 1),
...
-- ============================================================
-- 18. reporte_pedidos_por_tipo_venta
-- ============================================================
...
BEGIN
    ...
    WITH VentasActual AS (
        ...
        FROM order_items oi JOIN orders o ON oi.shopify_order_id = o.shopify_order_id
        WHERE o.created_at >= _col_date_boundary(7)
    ...
    VentasPasadas AS (
        ...
        FROM order_items oi JOIN orders o ON oi.shopify_order_id = o.shopify_order_id
        WHERE o.created_at >= _col_date_boundary(28)
          AND o.created_at < _col_date_boundary(7)
          AND UPPER(oi.category) NOT IN ('BOLSA', 'INSUMOS')
        GROUP BY oi.sku
    )
    SELECT c.image_url, c.title, c.sku, COALESCE(va.unidades, 0)::BIGINT,
        ...
        CASE
            ...
            ELSE '📊 ESTABLE'
        END
    FROM product_catalog c JOIN VentasActual va ON c.sku = va.sku LEFT JOIN VentasPasadas vp ON c.sku = vp.sku
    WHERE va.unidades > 5
...
-- ============================================================
-- 22. reporte_comportamiento_producto
-- ============================================================
...
-- ============================================================
-- 23. reporte_wos_categoria_tienda
-- ============================================================
...
BEGIN
    ...
    WITH VentasPeriodo AS (
        ...
        WHERE o.created_at >= v_boundary AND o.location_id = v_loc_id AND UPPER(c.category) NOT IN ('BOLSA', 'INSUMOS')
        GROUP BY UPPER(c.category)
    ),
...
-- ============================================================
-- 24. reporte_wos_categoria_global
-- ============================================================
...
BEGIN
    ...
    FROM (SELECT snapshot_date, COUNT(DISTINCT variant_id) as cnt FROM inventory_snapshot GROUP BY snapshot_date ORDER BY snapshot_date DESC) sub
    WHERE sub.cnt >= 5000 LIMIT 1;
    IF v_max_date_prendas IS NULL THEN SELECT MAX(snapshot_date) INTO v_max_date_prendas FROM inventory_snapshot; END IF;
...
-- ============================================================
-- 26. reporte_sugerencias_traslado
-- ============================================================
...
-- ============================================================
-- 27. reporte_curva_traslados
-- ============================================================
CREATE OR REPLACE FUNCTION public.reporte_curva_traslados(dias_atras integer DEFAULT 30, p_origen text DEFAULT NULL::text, p_destino text DEFAULT NULL::text)
...
DECLARE
  ...
  v_boundary timestamptz := _col_date_boundary(GREATEST(COALESCE(dias_atras, 30), 1));
BEGIN
  SELECT sub.snapshot_date INTO v_max_date
  ...
  ventas AS (
    ...
    FROM order_items oi JOIN orders o ON oi.shopify_order_id = o.shopify_order_id
    WHERE o.created_at >= v_boundary AND UPPER(COALESCE(oi.category, '')) NOT IN ('BOLSA', 'INSUMOS') AND oi.variant_id IS NOT NULL
  ...
  cat_part AS (
    ...
    FROM order_items oi JOIN orders o ON oi.shopify_order_id = o.shopify_order_id
    WHERE o.created_at >= v_boundary AND UPPER(COALESCE(oi.category, '')) NOT IN ('BOLSA', 'INSUMOS')
...
-- ============================================================
-- 28. reporte_desempeno_por_linea
-- ============================================================
...
-- ============================================================
-- 29. reporte_detalle_producto_tiendas
-- ============================================================
CREATE OR REPLACE FUNCTION public.reporte_detalle_producto_tiendas(dias_atras integer, p_producto text)
...
-- ============================================================
-- 30. reporte_detalle_skus_producto
-- ============================================================
...
-- ============================================================
-- 31. reporte_productos_por_categoria
-- ============================================================
...
AS $function$
DECLARE
  fecha_inicio TIMESTAMP WITH TIME ZONE := _col_date_boundary(GREATEST(COALESCE(dias_atras,1),1));
...
BEGIN
    ...
    WITH ConsumoGlobal AS (
        SELECT oi.sku, SUM(oi.quantity) / 30.0 as unidades_dia
        FROM order_items oi JOIN orders o ON oi.shopify_order_id = o.shopify_order_id
        WHERE o.created_at >= _col_date_boundary(30)
          AND (oi.category ILIKE '%bolsa%' OR oi.category ILIKE '%insumo%')
        GROUP BY oi.sku
    ),
    StockCEDI AS (
        SELECT inv.sku, SUM(inv.available)::BIGINT as stock
        FROM inventory_snapshot inv WHERE inv.location_id = '71474315479' AND inv.snapshot_date = v_max_date
        GROUP BY inv.sku
    )
    SELECT c.image_url, c.title, c.sku, COALESCE(s.stock, 0)::BIGINT,
        ROUND(COALESCE(v.unidades_dia, 0), 2), ROUND(COALESCE(s.stock, 0)::NUMERIC / NULLIF(v.unidades_dia, 0), 1),
        CASE WHEN ROUND(COALESCE(s.stock, 0)::NUMERIC / NULLIF(v.unidades_dia, 0), 1) < 15 THEN '🚨 REORDEN URGENTE'
             WHEN ROUND(COALESCE(s.stock, 0)::NUMERIC / NULLIF(v.unidades_dia, 0), 1) < 30 THEN '⚠️ PLANEAR COMPRA'
             ELSE '✅ STOCK SUFICIENTE' END
    FROM product_catalog c LEFT JOIN StockCEDI s ON c.sku = s.sku LEFT JOIN ConsumoGlobal v ON c.sku = v.sku
