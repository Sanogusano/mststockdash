...
BEGIN
    ...
    RETURN QUERY
    WITH
    -- Filter inventory FIRST (single scan across both target dates) BEFORE joining catalog
    InvRaw AS (
        SELECT inv.variant_id, inv.location_id, inv.snapshot_date, inv.available
        FROM inventory_snapshot inv
        WHERE inv.snapshot_date IN (v_max_date_prendas, v_max_date_insumos)
    ),
    CategoryMapping AS (
