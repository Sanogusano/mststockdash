-- 1. Limpiar espacios existentes en presupuestos_config
UPDATE presupuestos_config
SET nombre_identificador = TRIM(nombre_identificador),
    updated_at = now()
WHERE nombre_identificador <> TRIM(nombre_identificador);

-- 2. Constraint para impedir reintroducir espacios
ALTER TABLE presupuestos_config
DROP CONSTRAINT IF EXISTS presupuestos_config_nombre_trimmed;

ALTER TABLE presupuestos_config
ADD CONSTRAINT presupuestos_config_nombre_trimmed
CHECK (nombre_identificador = TRIM(nombre_identificador));

-- 3. Recrear get_centro_accion_comercial filtrando tipo IN ('tienda','canal')
...
BEGIN
  ...
  ordenes_base AS (
    ...
    WHERE o.created_at >= v_inicio_ts AND o.created_at < v_fin_ts AND o.financial_status IN ('paid', 'partially_refunded', 'partially_paid')
  ...
  pres AS (
    ...
    WHERE pc.anio = p_anio AND pc.mes = p_mes AND pc.monto > 0
      AND pc.tipo IN ('tienda', 'canal')  -- excluye presupuestos por vendedor (van al módulo de comisiones)
  ),
  stmp AS (
...
-- 4. Documentar la RPC
COMMENT ON FUNCTION public.get_centro_accion_comercial(integer, integer) IS
'Dashboard de cumplimiento por tienda y canal. Los presupuestos por vendedor se manejan en el módulo de comisiones.';